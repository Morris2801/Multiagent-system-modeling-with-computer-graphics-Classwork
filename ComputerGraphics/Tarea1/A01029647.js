...
import { shapeF, generateData, caraFeliz } from './shapes.js';
...
function drawScene(gl, vao, programInfo, bufferInfo) {
    ...
    uniforms.u_color = [0, 0, 0, 1]; // Negro
    twgl.setUniforms(programInfo, uniforms);
    gl.drawElements(gl.TRIANGLES, 60, gl.UNSIGNED_SHORT, 300 * 2); // offset = 300 índices × 2 bytes
    
    uniforms.u_color = [0, 0, 0, 1]; // Negro
    twgl.setUniforms(programInfo, uniforms);
    gl.drawElements(gl.TRIANGLES, 60, gl.UNSIGNED_SHORT, (300 + 60) * 2);
    
    // BOCA ROJA (smileLen lados + 1 centro → smileLen triángulos × 3 índices)
    // Con smileLen=40: 40 triángulos × 3 = 120 índices
    uniforms.u_color = [1, 0, 0, 1]; // Rojo
    twgl.setUniforms(programInfo, uniforms);
    let mouthIndices = 150; 
    gl.drawElements(gl.TRIANGLES, mouthIndices, gl.UNSIGNED_SHORT, (300 + 60 + 60) * 2);
